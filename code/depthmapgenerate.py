...
r= cv2.imread('r.jpg', cv2.IMREAD_GRAYSCALE)
...
# Resize the image
left_image  = cv2.GaussianBlur(cv2.resize(l, dim, interpolation=cv2.INTER_AREA), (7,7), 0)
right_image = cv2.GaussianBlur(cv2.resize(r, dim, interpolation=cv2.INTER_AREA), (7,7), 0)
...
initial_focal_length = 4539  # Initial focal length in pixels
